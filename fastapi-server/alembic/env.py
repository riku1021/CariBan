...
# alembic は別プロセスで起動するため、pytest と同様に fastapi-server/ を import ルートにする
_SERVER_ROOT = Path(__file__).resolve().parent.parent
if str(_SERVER_ROOT) not in sys.path:
    sys.path.insert(0, str(_SERVER_ROOT))

from src.infrastructure.config.settings import load_settings  # noqa: E402
...
def to_sync_database_url(url: str) -> str:
    """Alembic は同期接続のため、asyncpg URL を同期ドライバ（psycopg3）用に置き換える。"""
